...
class RMGRec_I(nn.Module):
    def __init__(self, opt, n_node, params, time_A, region_A, time_out, region_out):
        ...
        self.gat_freq = gat_cell(self.hidden_size)
        self.gat_t = gat_cell(self.hidden_size)
        self.gat_loc = gat_cell(self.hidden_size)
    ...
    def forward(self, lens, a, A_freq, A_time, A_loc, items, mask, cross_s, inputs, time_seq, loc_seq):
        ...
        hidden_p_freq = self.gat_freq(hidden, A_freq).unsqueeze(1)
        hidden_p_t = self.gat_t(hidden, A_time).unsqueeze(1)
        hidden_p_loc = self.gat_loc(hidden, A_loc).unsqueeze(1)
        ...
        for idx, l in enumerate(lens):
            ...
            mk = torch.stack(
                [torch.cat([TMK[self.timeslot.index(i)], SMK[self.region.index(j)]], dim=0) for i, j in zip(seq_t, seq_r)])
