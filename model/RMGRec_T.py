...
class RMGRec_T(nn.Module):
    def __init__(self, opt, n_node, params, time_A, region_A, time_out, region_out):
        ...
        self.gat_freq = he_gat(self.hidden_size)
        self.gat_t = he_gat(self.hidden_size)
        self.gat_loc = he_gat(self.hidden_size)
        ...
        self.mk_learner_t = mk_learner(self.hidden_size, 2*self.meta_size).to("cuda")
    ...
    def forward(self, lens, a, A_freq, A_time, A_loc, items, mask, cross_s, inputs, time_seq, loc_seq):
        ...
        for idx, l in enumerate(lens):
            ...
            mk = torch.stack([TMK[self.timeslot.index(i)] for i in seq_t])
